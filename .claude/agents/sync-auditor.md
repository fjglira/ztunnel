...
2. **Feature flags**: OpenSSL/FIPS changes gated with `#[cfg(feature = "openssl-tls")]`.
3. **Upstream-first**: changes that belong upstream have a corresponding upstream PR or issue.
4. **Build health**: `cargo build`, `cargo test`, `cargo test --features openssl-tls`, and `cargo clippy -- -D warnings` all pass.
